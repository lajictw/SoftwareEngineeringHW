...
<body>
	...
	<table border="3" width="500" bgcolor="aqua" align="center" cellpadding="5" cellspacing="1">
		...
		for ($i = 0; $i < count($courseId); $i++) {
			...
			<select name=\"firstclass\">
			";
			for($j=1;$j<=7;$j++){
				if($j==$firstclass[$i])
					echo"<option value=$j>$days[$j](原定)</option>";
				else
					echo"<option value=$j>$days[$j]</option>";
			}
			if($firstclass[$i]==0)
				echo"<option value=$j>无(原定)</option>";
			echo'</td>';
			//secondclass
			echo "
			<td>
			<form action=\"logic/edit.php\" method=\"get\">
			<select name=\"secondclass\">
			";
			for($j=1;$j<=7;$j++){
				if($j==$secondclass[$i])
					echo"<option value=$j>$days[$j](原定)</option>";
			...
			for($j=1;$j<=5;$j++){
				if($j==$classTime[$i])
					echo"<option value=$j>$times[$j](原定)</option>";
				else
					echo"<option value=$j>$times[$j]</option>";
			}
			echo'</td>';
			echo'<form action="logic/edit.php" method="get">';
			echo"<td><button value=$courseId[$i] name= \"change\" type=\"submit\">更改</button></td>";
			echo '</tr>';
		}
		?>
		
		<?php
			echo "<td><input type=text name=\"name\" style=\"width:80px; height:20px;\"><td>";
			//newclass1
			echo "
			<form action=\"logic/edit.php\" method=\"get\">
			<select name=\"newclass1\">
			";
			for($j=1;$j<=7;$j++){
				echo"<option value=$j>$days[$j]</option>";
			}
			...
			echo'</td>';
			echo'<form action="logic/edit.php" method="get">';
			echo"<td><button value=$newId name= \"add\" type=\"submit\">添加</button></td>";
			echo '</tr>';
		?>
